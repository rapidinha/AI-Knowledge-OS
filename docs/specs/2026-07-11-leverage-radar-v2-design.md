...
| `topics.yaml` | Cross-day topic memory — machine index (private, gitignored) |
| `topics/<slug>.md` | Obsidian topic notes with **Rolling summary** (durable; future AI summarization) |
| `topics/_index.md` | Topic MOC for Obsidian |
| `topics.example.yaml` / `topic.md` | Public templates |
| Daily template | Recurrence + wikilink to topic note |
| `radar/lib/topics_io.py` | Load/save `topics.yaml` only — no ML |

---

## 4. Topic graph (dual-write)
...
### Obsidian / future AI surface

Path: `journals/radar/topics/<slug>.md` (+ `_index.md` MOC).

Each note keeps a **Rolling summary**, **Timeline** (with wikilinks to daily radar notes), and **Sources**. These files are durable vault memory: a later Obsidian or vault AI pass should be able to summarize a theme from the topic note alone, without `_raw/` history. Do **not** delete notes on retire — set `status: retired`. Private SoT may commit topic Markdown; never promote `journals/` to public upstream.
...
4. Cap ~200 topics; mark oldest low-hit as `retired` when over cap (**keep** Markdown notes).
5. Missing/corrupt `topics.yaml` → recreate empty `topics: []` and note in Executive Summary.
6. Every touched topic **dual-writes** YAML index + Markdown note (update rolling summary).
