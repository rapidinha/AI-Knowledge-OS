...
1. Copy `templates/radar/config.example.yaml` → `raw/ops/radar/config.yaml` (private, gitignored).
...
1. Set `providers.lobsters.enabled: true` in `raw/ops/radar/config.yaml`.
...
python radar/providers/fetch_enabled.py \
  ...
  --out raw/ops/radar/_raw/2026-07-11.jsonl
```

Individual providers can also be run for debugging:

```bash
python radar/providers/hn/fetch.py --out raw/ops/radar/_raw/hn.jsonl
```
